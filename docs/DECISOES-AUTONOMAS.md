...
| 9 | 20/07 | **"liga"** → apliquei o fix retido (bug-1): ligar a config da janela de encomendas ao motor, **só no modo live** (demo/CSV preservados). `loadWeekData` lê `getConfig` e passa a janela a `fetchLiveOrders`. 394 testes. | Interpretei "liga" como a luz verde para "ligar a config à janela" (a palavra que usei ao retê-lo). Comportamento idêntico até o operador mudar o cutoff (default = mesmos valores). Limitação `ignoreAfterClose=false` documentada em POR-FAZER. |
| 10 | 28/07 | Documento **"Rotas de câmara"** (00. Rotas.xlsx do Miguel) a ser construído (motor+impressão+export). | Contribui para o projeto (é 1 dos docs semanais que o Miguel faz à mão); dados já no motor; o António deu luz verde ("se contribuir, sim"). |
| 11 | 28/07 | **Mandato alargado** (António): "faz tudo o que acelere deploy/entrega e facilite o Shopify". A executar: (a) ligação por **token estático** via env (plug-and-play quando as credenciais chegarem); (b) `fly.toml` + `.env.example` (deploy num comando); (c) manual do operador. | Reduz a fricção do dia-do-deploy e do dia-em-que-as-credenciais-chegam ao mínimo. Tudo reversível/interno; sem tocar em nada externo. |
| 12 | 28-29/07 | **Lote interno** (António, via supervisão): CI verde; de-risking Postgres (DDL offline); e as 4 prioridades — pós-fecho honrado, histórico de semanas, deploy prep+checklist, envio de emails agnóstico (dry-run). 445 testes. | Objetivo: deixar o trabalho Loop reduzido a "só integração real + piloto". Email é DRY-RUN (nada enviado — respeita "nada virado para fora"). |
